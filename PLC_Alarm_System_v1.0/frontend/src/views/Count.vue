<template>
  <div class="sany-content">
    ...
    <div class="sany-section-title">{{ plcTitle }} TOP 10 故障 — 次数对比</div>
    <div style="display:flex;gap:20px;">
      <div style="flex:1;min-width:0;">
        <div class="sany-card">
          <div class="sany-card-header">
            <span>本周 vs 上周报警次数</span>
        ...
        <div class="sany-card">
          ...
          <div class="sany-card-body" style="padding:0;overflow-x:hidden;">
            <table class="sany-table" style="table-layout:fixed;width:100%;">
              <thead><tr><th style="width:42px;">代码</th><th style="width:82px;">描述</th><th style="width:44px;">本周</th><th style="width:44px;">上周</th><th style="width:52px;">趋势</th><th style="width:28px;"></th></tr></thead>
              <tbody>
                <tr v-for="item in displayList" :key="item.alarm_code">
                  ...
                  <td style="font-size:0.75rem;">{{ item.this_week_count }}</td>
                  <td style="font-size:0.75rem;">{{ item.last_week_count }}</td>
...
import { getWeeklyReport, exportExcel } from '../api/statistics'
...
const getIndicator=item=>{const tv=item.this_week_count;const lv=item.last_week_count;if(tv===0&&lv>0)return'ind-green';if(lv>0&&tv<lv*0.5)return'ind-yellow';return'ind-red'}

const loadData=async()=>{
  const monday=getMonday(selectedWeek.value)
  const r=await getWeeklyReport(monday.toISOString().slice(0,10),selectedPlcId.value,50)
  rawData=r.data.top_alarms;sortData()
}
const sortData=()=>{
  const s=[...rawData];s.sort((a,b)=>sortBy.value==='thisWeek'?b.this_week_count-a.this_week_count:b.last_week_count-a.last_week_count)
  displayList.value=s.slice(0,10);updateChart()
}
const updateChart=()=>{
  if(!chart)return;const d=displayList.value
  chart.setOption({
    tooltip:{trigger:'axis',formatter:p=>{const i=d.find(x=>x.alarm_code===p[0].axisValue);let h=`<b>${p[0].axisValue}</b> — ${i?.alarm_message||''}<br/>`;p.forEach(x=>{h+=`${x.marker}${x.seriesName}:${x.value}<br/>`});return h}},
    xAxis:{data:d.map(x=>x.alarm_code),axisLabel:{color:'#666',rotate:30}},yAxis:{axisLabel:{color:'#666'},splitLine:{lineStyle:{color:'#eee'}}},
    grid:{bottom:60},
    series:[{name:'本周',type:'bar',data:d.map(x=>x.this_week_count),itemStyle:{color:'#C41230'}},{name:'上周',type:'bar',data:d.map(x=>x.last_week_count),itemStyle:{color:'#DDD'}}]
  })
  chart.off('click');chart.on('click',p=>{const i=d.find(x=>x.alarm_code===p.name);if(i)alert(`报警:${i.alarm_code}\n描述:${i.alarm_message}\n本周:${i.this_week_count}\n上周:${i.last_week_count}\n变化:${i.change_percent}%`)})
}
const exportReport=async()=>{
  const monday=getMonday(selectedWeek.value);const r=await exportExcel(monday.toISOString().slice(0,10),selectedPlcId.value,'count')
  const blob=new Blob([r.data],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'})
  const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`${plcTitle.value}_W${getWeekNumber(monday)}_TOP10故障列表（次数）.xlsx`;a.click();URL.revokeObjectURL(a.href)
}
